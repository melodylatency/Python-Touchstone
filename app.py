...
class Weather(Tk):
    ...
    def __gui(self):
        ...
        Label(text="Current Weather :", font="Arial 14 bold", fg="red").place(
            x=590, y=7
        )
        ...
        self.finalimg4 = ImageTk.PhotoImage(
            image=Image.open(r"images/sunrise.png").resize((40, 40))
        )
        ...
        self.finalimg5 = ImageTk.PhotoImage(
            image=Image.open(r"images/sunset.png").resize((40, 30))
        )
        ...
        self.finalimg6 = ImageTk.PhotoImage(
            image=Image.open(r"images/bottom_bar.png").resize((770, 70))
        )
        Label(image=self.finalimg6, bg="#00b7ff").place(x=5, y=330)

        Label(text="Humidity", font="Calibri 15 bold", bg="#00b7ff", fg="white").place(
            x=35, y=335
        )
        Label(text="Pressure", font="Calibri 15 bold", bg="#00b7ff", fg="white").place(
            x=210, y=335
        )
        Label(
            text="Description", font="Calibri 15 bold", bg="#00b7ff", fg="white"
        ).place(x=400, y=335)
        Label(
            text="Visibility", font="Calibri 15 bold", bg="#00b7ff", fg="white"
        ).place(x=600, y=335)

        self.humidity = Label(
            text="", font=("Calibri", 15, "bold"), bg="#00b7ff", fg="black"
        )
    ...
    def __set_information(self, weather):
        if weather["cod"] == "404" and weather["message"] == "city not found":
            ...
            self.feel["text"] = (
                f"Feels Like {int(weather['main']['feels_like']-273)}° | {weather['weather'][0]['main']}"
            )
    ...
    def exit(self):
        a = messagebox.askyesno("Confirmation", "Exit?")
        if a == True:
            self.destroy()
